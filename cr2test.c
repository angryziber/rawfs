...
void write_jpeg(int fd, const char *filename, struct img_data *img) {
    find_thumb(fd, img);
	...
	*(short*)&outp[4] = htons(8 + img->ifd2_offset);  // exif data size in the APP1 marker
	...
	int ifd_leng    th = sizeof img->ifd_size + img->ifd_size * sizeof(struct tiff_tag);
	*(int*)&outp[CR2_HEADER_LENGTH + ifd_length] = 0;
	...
	write_file(filename, img->out, img->out_length);
	free(img->out);
}
...
int main(int argc, char* argv[]) {
    ...
    if (fd != -1) {
        write_jpeg(fd, "thumb_exif.jpg", &img);
